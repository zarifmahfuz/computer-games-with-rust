...
#[derive(Debug)]
pub enum Msg {
    setPlayerName1(InputEvent),
    setPlayerName2(InputEvent),
...
impl Component for TOOTHuman {
    ...
    fn create(_ctx: &Context<Self>) -> Self {
        ...
        TOOTHuman {
            ...
            difficulty: 1,
            TO: 1,
            name_callback1: _ctx.link().callback(|e: InputEvent| Msg::setPlayerName1(e)),
            name_callback2: _ctx.link().callback(|e: InputEvent| Msg::setPlayerName2(e)),
        ...
        match _msg {
            ...
            Msg::clicked(col) => {
                if self.winner == 0{
                    ...
                    if col.is_some(){
                        if self.game.clone().borrow().to_move == -1{
                        ...
                        else if self.game.clone().borrow().to_move == 1{
                            let temp_col = col.unwrap() as usize;
                            self.new_select_col = temp_col;

                            let temp = self.game.clone().borrow_mut().player_2_move(temp_col, self.TO);
                            if temp.0 == -1 {
                                log::info!("column full");
                            }
                            else {
                                computer_draw(self.game.clone(), temp_col, self.TO);
                                self.winner = self.game.clone().borrow().check_winner();
                            }
                        }
                        if self.winner == -1 {
    ...
    fn view(&self, _ctx: &Context<Self>) -> Html {
        ...
        html! {
            ...
            <div class="col-md-offset-4 col-md-8">
                ...
                <p>{format!("New Game:  {} Vs {}",self.player1, self.player2)}</p>
            </div>
            }
            }
            else {
            ...
            <div class="post" ng-repeat="game in games">
                <br/>
                <small>{format!("(Winning Combination: {} - ", self.player1)} <b>{"TOOT"}</b> {format!("   and    {} - ", self.player2)} <b>{"OTTO)"}</b></small>
                <br/>
                {"Select a Disc Type:  "}
